...
| A new attribution check that flagged 30 of 422 CVEs as suspicious | It asked "is there a Microsoft keyword near this CVE?" — and real product names contain no keyword any list would hold: `Storvsp.sys Driver`, `Kernel Streaming WOW Thunk`, `Routing and Remote Access Service`. **Absence of a keyword is not evidence.** A check that cries wolf thirty times is worse than no check, because it is still in the code to be pointed at afterwards. Rewritten to fire only on positive evidence of another vendor, which took it from 30 findings to 1 |
| A CVE ID pattern bounded at seven digits | The ID syntax has a four-digit *minimum* and no maximum. `CVE-2026-12345678` matched as `CVE-2026-1234567` — a well-formed identifier for a **different vulnerability**, which then found no scanner mapping and was reported as coverage-unverified. A bound that truncates is worse than one that rejects |
| My own reasoning: "a four-digit CVE suffix is suspicious" | Sequence numbers reflect when a CNA reserved the block, nothing else. I called a legitimate `Windows TPM` CVE anomalous on the strength of a digit count, and two of my three supporting "tells" were equally empty. The operator corrected it. Pattern-matching on incidental structure feels like evidence and is not |
