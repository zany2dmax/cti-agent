...
func TestAttributionIsConfiguredNotHardcoded(t *testing.T) {
	// This repo was deliberately genericised - no org name, no addresses, no
	// internal identifiers. An attribution line baked into the renderer would
	// undo that, so unset means the footer keeps the wording it always had and
	// advertises nobody.
	...
	r := &Report{Digest: parsed(t), Org: "Construction Resources"}
	if !strings.Contains(r.Text(), "Generated by the Construction Resources CTI agent fleet") {
		t.Error("with nothing configured the old footer must survive")
	}
	...
	// its own line in text.
	r = &Report{Digest: parsed(t), Org: "CR", Attribution: a}
	h := r.HTML()
	if !strings.Contains(h, `href="https://github.com/example/cti-agent"`) {
		t.Errorf("HTML footer missing the link:\n%s", h)
	}
	if !strings.Contains(h, "CR Claude Code Agent Fleet") {
		t.Error("HTML footer missing the credit text")
	}
	txt := r.Text()
	if !strings.Contains(txt, "Correlated and published by the CR Claude Code Agent Fleet") ||
		!strings.Contains(txt, "\nhttps://github.com/example/cti-agent") {
		t.Errorf("text footer wrong:\n%s", txt)
	}
}

func TestOnlyHTTPLinksReachTheEmail(t *testing.T) {
	// The footer is rendered as HTML in Outlook and the URL comes from a
	// config file. A javascript: or data: href is at best broken and at worst
	// a payload; checking the scheme costs nothing.
	for _, bad := range []string{
		"javascript:alert(1)",
		"data:text/html;base64,PHNjcmlwdD4=",
		"file:///etc/passwd",
		"github.com/example/cti-agent", // no scheme
		"https://",                     // no host
		"  ",
	} {
		if got := safeLinkURL(bad); got != "" {
			t.Errorf("safeLinkURL(%q) = %q, want it refused", bad, got)
		}
	}
	for _, ok := range []string{
		...
		if safeLinkURL(ok) == "" {
			t.Errorf("safeLinkURL(%q) refused a usable link", ok)
		}
	}

	// A refused URL must not become a link, and must not silently take the
	// credit text with it.
	t.Setenv("FLEET_ATTRIBUTION", "Published by the CR fleet")
	t.Setenv("FLEET_REPO_URL", "javascript:alert(1)")
	a := LoadAttribution("CR")
	if a.URL != "" {
...
func TestAttributionTextIsEscapedInHTML(t *testing.T) {
	...
	if strings.Contains(h, "<script>") {
		t.Errorf("unescaped script in the footer: %s", h)
	}
	if strings.Contains(h, `b="2"`) {
		t.Errorf("unescaped quote in the href would break the attribute: %s", h)
	}
	if !strings.Contains(h, "&amp;") {
		t.Errorf("ampersand not escaped: %s", h)
	}
}
